...
export class Node {
      ...
      this.node.handle([`/port/${username}`], ({ stream }) => {
        pipe(stream, async function (source) {
          for await (const msg of source) {
            ...
            console.log(`from: ${stream.stat.protocol}, msg: ${str}`);
  ...
  post = (message) => {
    ...
    this.node.pubsub.publish(topic, str2array(JSON.stringify(messageObject)));
  };
  ...
  follow = async (username) => {
    ...
    try {
      ...
      if (peerId) {
        ...
        const posts = this.handleReceivePosts(username);
