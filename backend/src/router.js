...
const feedHandler = async (node, req, res) => {
    let feed = "WRONG"
    if(node.port !== 3001) feed = "piu piu piu piu piu piu piu piu piu piu piu piu piu a lot"
  res.status(200).json({
...
export const setupRoutes = (node, app) => {
  ...
  app.post("/register", (req,res) => {registerHandler(node,req,res)});
  app.post("/login", (req,res) => {loginHandler(node,req,res)});
  ...
  app.get("/feed", (req,res) => {feedHandler(node,req,res)});
